...
class LaravelExcelReader
{
	 ...
	 * @var array
	 */
	public $filters = array('registered' => array());
	...
	{
		// Set the extension
		$this->_setFile($file)->setExtension()->setTitle()->_setFormat()->_setReader()->_enableFilters()->_setInputEncoding($encoding);
	}
	...
	protected function _setReaderDefaults()
	{
		...
		if ($this->format == 'CSV') {
			...
			if (!$this->delimiter)
				$this->reader->setDelimiter(Config::get('excel::csv.delimiter', ',')); else
				$this->reader->setDelimiter($this->delimiter);

			if (!$this->enclosure)
				$this->reader->setEnclosure(Config::get('excel::csv.enclosure', '"')); else
