...
class LaravelExcelWorksheet extends PHPExcel_Worksheet
{
	 ...
	 * @var array
	 */
	public $allowedPageSetup = array('orientation', 'paperSize', 'scale', 'fitToPage', 'fitToHeight', 'fitToWidth', 'columnsToRepeatAtLeft', 'rowsToRepeatAtTop', 'horizontalCentered', 'verticalCentered', 'printArea', 'firstPageNumber');
	 ...
	 * @var array
	 */
	public $allowedStyles = array('fontFamily', 'fontSize', 'fontBold');
	...
	{
		// Set all borders
		$this->getStyle($pane)->getBorders()->getAllBorders()->setBorderStyle($weight);
	...
	{
		$styleArray = array('borders' => array('allborders' => array('style' => $weight)));

		// Apply the style
		$this->getDefaultStyle()->applyFromArray($styleArray);
	...
	public function setColumnFormat(Array $formats)
	{
		...
		foreach ($formats as $column => $format) {
			...
			$this->getStyle($column)->getNumberFormat()->setFormatCode($format);
		}
	...
	{
		is_numeric($cellValue) && !is_integer($cellValue) ? $this->getCell($currentColumn . $startRow)->setValueExplicit($cellValue) : $this->getCell($currentColumn . $startRow)->setValue($cellValue);
	}
}