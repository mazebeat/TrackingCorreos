...
class LaravelExcelWriter
{
	...
	protected function _download()
	{
		...
		$this->_setHeaders(array(
			...
			'Content-Type'  => $this->contentType, 'Content-Disposition' => 'attachment; filename="' . $this->filename . '.' . $this->ext . '"', 'Expires' => 'Mon, 26 Jul 1997 05:00:00 GMT', // Date in the past
			'Last-Modified' => Carbon::now()->format('D, d M Y H:i:s'), 'Cache-Control' => 'cache, must-revalidate', 'Pragma' => 'public'
	...
	public function store($ext = 'xls', $path = false, $returnInfo = false)
	{
		...
		if ($this->returnInfo($returnInfo)) {
			...
			return array('full' => $toStore, 'path' => $this->storagePath, 'file' => $this->filename . '.' . $this->ext, 'title' => $this->filename, 'ext' => $this->ext);
		}
