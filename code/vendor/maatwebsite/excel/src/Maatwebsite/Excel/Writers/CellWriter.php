...
class CellWriter
{
	...
	{
		return $this->setStyle('font', array('name' => $family));
	}
	...
	{
		return $this->setStyle('font', array('size' => $size));
	}
	...
	{
		return $this->setStyle('font', array('bold' => ($bold == 'bold' || $bold) ? true : false));
	}
	...
	public function setBorder($top = 'none', $right = 'none', $bottom = 'none', $left = 'none')
	{
		...
		$styles = is_array($top) ? $top : array('top' => array('style' => $top), 'left' => array('style' => $left,), 'right' => array('style' => $right,), 'bottom' => array('style' => $bottom,));
	...
	{
		return $this->setStyle('alignment', array('horizontal' => $alignment));
	}
	...
	{
		return $this->setStyle('alignment', array('vertical' => $alignment));
	}
	...
	{
		// Set the styles
		$styles = is_array($color) ? $color : array('type' => $type, 'color' => array($colorType => str_replace('#', '', $color)));
	...
	protected function setStyle($styleType, $styles)
	{
		...
		$style->applyFromArray(array($styleType => $styles));
