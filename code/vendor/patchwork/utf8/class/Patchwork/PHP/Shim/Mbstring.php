...
class Mbstring
{
	...
	protected static
		...
		'UTF-8'), $language = 'neutral', $internal_encoding = 'UTF-8', $caseFold = array(array('µ',
		'ſ',
		"\xCD\x85",
		'ς',
		"\xCF\x90",
	...
	static function mb_convert_case($s, $mode, $encoding = INF)
	{
		...
		if (INF === $encoding)
			$encoding = self::$internal_encoding; else $encoding = strtoupper($encoding);

		if ('UTF-8' === $encoding || 'UTF8' === $encoding)
			$encoding = INF; else $s = iconv($encoding, 'UTF-8//IGNORE', $s);
		...
		if (MB_CASE_TITLE == $mode) {
			...
			while ($i < $len) {
				...
				if (isset($map[$uchr])) {
					...
					if ($nlen == $ulen) {
						$nlen = $i;
						do $s[--$nlen] = $uchr[--$ulen]; while ($ulen);
					} else {
		...
		if (INF === $encoding)
			return $s; else return iconv('UTF-8', $encoding, $s);
	}

	static function mb_internal_encoding($encoding = INF)
	{
		if (INF === $encoding)
			return self::$internal_encoding; else $encoding = strtoupper($encoding);
	...
	static function mb_detect_encoding($str, $encoding_list = INF, $strict = false)
	{
		if (INF === $encoding_list)
			$encoding_list = self::$encoding_list; else {
	...
	static function mb_substr($s, $start, $length = null, $encoding = INF)
	{
		...
		if (null === $length)
			$length = 2147483647; else if ($length < 0) {
	...
	static function mb_strstr($haystack, $needle, $part = false, $encoding = INF)
	{
		...
		if (false === $pos)
			return false;
		if ($part)
			return substr($haystack, 0, $pos); else return substr($haystack, $pos);
	}
	...
	protected static function getSubpart($pos, $part, $haystack, $encoding)
	{
		...
		if (false === $pos)
			return false;
		if ($part)
			return self::mb_substr($haystack, 0, $pos, $encoding); else return self::mb_substr($haystack, $pos, null, $encoding);
	}
