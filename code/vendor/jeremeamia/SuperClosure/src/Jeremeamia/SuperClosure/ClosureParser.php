...
class ClosureParser
{
	 ...
	 * parsings of the same file. If you are worried about the memory consumption of files the ClosureParser has
	 * already parsed, you can call this function to clear the cache. The cache is not persistent and stores ASTs
	...
	/**
	 * Returns the abstract syntax tree (AST) of the closure's code. Class names are resolved to their
	 * fully-qualified class names (FQCN) and magic constants are resolved to their values as they would be in the
	...
	/**
	 * Loads the PHP file and produces an abstract syntax tree (AST) of the code. This is stored in an internal
	 * cache by the filename for memoization within the same process
	 *
	 * @return array
	 */
